...
export default function BlogViewUser(props)
{  
    ...
    {   
        ...
        const response = await fetch("http://localhost:8080/blogs/removeComment", {
            method: "POST",
            mode: "cors",
            headers: {
    ...
    async function removeIssue(issue_id)
    {
        const response = await fetch("http://localhost:8080/blogs/removeIssue", {
            method: "POST",
            mode: "cors",
            headers: {
              "Content-Type": "application/json",
    ...
    <p>{comment.comment}</p>
    <button type="button" class="btn btn-danger" onClick={()=>{removeComment(comment._id)}}>Remove</button>
    <hr/></>; }
      else{
        return <></>;
      }
    });
    let Issues = blog.issues.map(issue => {
        if(issue.name !== undefined){
        return <> <div className="row">
        <div className="col-md-7"> <h5>{issue.name}</h5></div>
        <div className="col-md-5"> <h6>{new Date(issue.date).toDateString()}</h6></div>
    
    </div>
    <p>{issue.issue}</p>
    <button type="button" class="btn btn-danger" onClick={()=>{removeIssue(issue._id)}}>Remove</button>
    <hr/></>; }
      else{
        return <></>;
      }
    ...
    transform: translate(-50%, 0px); font-size:17px;">  
              ...
              <i className="fa-solid fa-info" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRightissue" aria-controls="offcanvasRight"></i>&nbsp; {blog.issues.length-1}
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRightissue" aria-labelledby="offcanvasRightLabel">
